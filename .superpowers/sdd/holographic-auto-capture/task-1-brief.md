# Task 1: Create capture.py — CaptureEngine

## Files
- Create: `plugins/memory/holographic/capture.py`
- Test: `tests/plugins/memory/holographic/test_capture.py`

## Interfaces
- Consumes: `MemoryStore.add_fact(content, category, tags)`, `ctx.llm.complete(messages)`
- Produces: `CaptureEngine(buffer, store, llm, interval)` with `observe_turn(messages)` and `compress_and_store()` methods
...
- New files only (no modifications to existing files in this task)
